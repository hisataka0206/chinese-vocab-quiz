...
スタート画面で「パスワードを入力して復習モードへ」ボタンを押すと、過去の履歴から出題する復習モード画面に移動します。

- パスワード(allowlist のいずれか)を入力してから遷移する必要があります。
- GAS 経由で `Details` シートの該当ユーザーの行を取得し、**単語ごとに最新の試行**をとりまとめます。
- 「間違えた問題」モードでは最新の試行で不正解だった単語のみ、「正解した問題」モードでは最新の試行で正解だった単語のみから出題されます。
- 4 択のダミー選択肢は通常モードと同じく全辞書から引きます。
- 結果は通常モードと同様に `Summary` (+ allowlist 対象であれば `Details`) に記録されます。

バックエンドは `doPost` が `action: "fetch"` を受けた場合に履歴を返す実装になっており、allowlist 以外の名前では `not_allowed` が返ります。
